 ...
 * Description P高客单价分析
 */
class RfmPrice extends Common
{
    /**
     * 按门店查询P客单价分析
     */
    public function priceSel()
    {
        // 获取数据
        ...
        if ($search != '') {
            // 统计数量
            $count = Db::table($db . '.vip_rfm_p')
                ->where('store_code', $search)
                ->count();
            // 查询的数据
            $data = Db::table($db . '.vip_rfm_p')
                ->where('store_code', $search)
                ->order('p_create_time', 'desc') // 按照时间降序排列
                ->page($page, $limit)
                ->select();
            // 周期
            ...
            //清除变量
            unset($page, $limit, $search, $db, $vipa, $p_consumption);
        } else {
            $count = 0;
            $data = [];
        }
    ...
    public function priceSelx()
    {
        ...
        if ($search != '') {
            ...
            $data = Db::table($db . '.vip_rfm_p')
                ...
                ->select();
            //周期
            $p_consumption = Db::table($db . '.vip_rfm_days')->field('p_consumption')
                ->where('store_code', $search)
                // ->where('store_code', 'MDERP1548127744152')
                ->find();
            // 查询人数需要条件
            ...
            //查询人数需要条件(金额)
            $fc_money = Db::table($db . '.vip_goods_order')->field('sum(real_pay) as pay')->where('status', 0)->where('vip_code', 'in', $cards)->group('vip_code')->select();
            //查询人数需要条件(次数)
            ...
            foreach ($data as $k => $v) {
                ...
                if ($data[$k]['p_update_time'] == "") {
                    $data[$k]['p_update_time_g'] = date('Y-m-d H:i:s', $v['p_create_time']);
                } else {
            ...
            //清除变量
            unset($page, $limit, $search, $db, $vips, $p_consumption, $cards, $c, $fctt, $fct, $n, $fc_frequency, $fc_money);
        } else {
            $count = 0;
            $data = [];
        }
    ...
    public function lookPeople()
    {
        ...
        // 查询的会员数据
        $vipData = rfma::rfmMember($id, $store, $staff_code, $db, 'p');
        // 基本配置
    ...
    public function messageMember()
    {
        ...
        // 查询的会员数据
        $vipData = rfma::rfmMember($id, $store, $staff_code, $db, 'p', input('vvip'));
        //提取会员的手机号并更改格式

 ...
 * Description C高消费分析
 */
class RfmConsumption extends Common
{
    /**
     * 按门店查询C高消费分析
     */
    public function consumptionSel()
    {
        //获取数据
        ...
        if ($search != '') {
            //统计数量
            $count = Db::table($db . '.vip_rfm_c')
                ->where('store_code', $search)
                ->count();
            //查询的数据
            $data = Db::table($db . '.vip_rfm_c')
                ->where('store_code', $search)
            ...
            //清除变量
            unset($page, $limit, $search, $db, $vipa, $c_consumption);
        } else {
            $count = 0;
            $data = [];
        }
    ...
    public function consumptionSelx()
    {
        //获取数据
        ...
        if ($search != '') {
            ...
            // 查询人数需要条件
            $fc = Db::table($db . '.vip_goods_order')
            ...
            foreach ($data as $k => $v) {
                ...
                if ($data[$k]['c_update_time'] == "") {
                    $data[$k]['c_update_time_g'] = date('Y-m-d H:i:s', $v['c_create_time']);
                } else {
            ...
            //清除变量
            unset($page, $limit, $search, $db, $vips, $c_consumption, $cards, $c, $fc, $fct, $n);
        } else {
            $count = 0;
            $data = [];
        }
    ...
    public function lookPeople()
    {
        ...
        // 查询的会员数据
        $vipData = rfma::rfmMember($id, $store, $staff_code, $db, 'c');
        //基本配置
    ...
    public function messageMember()
    {
        ...
        // 查询的会员数据
        $vipData = rfma::rfmMember($id, $store, $staff_code, $db, 'c', input('vvip'));
        //提取会员的手机号并更改格式

 ...
 * Description 生日模板
 */
class WreBirthday extends Common
{
    /**
     * 查询生日模板
     */
    public function WreBirthdayList()
    {
        //分页信息
        ...
        //统计数量
        $count = Db::table($db . '.vip_inbirthday_interaction')
            ->alias('v')
            ...
            ->count();
        //所需数据
        $data = Db::table($db . '.vip_inbirthday_interaction')
            ->alias('v')
            ->leftJoin($db . '.vip_org vg', 'vg.code = v.org_code')
            ->field('v.*,vg.name vgname')
            ->where($where)
     ...
     * 生日互动记录
     */
    public function WreBirthdayRecord()
    {
        //生成单号
        $foot = 'SRRHD' . str_replace('.', '', microtime(1));
        //按单号查询模板
        $birthday = Db::table($this->db . '.vip_inbirthday_interaction')->where('code', input('template_code'))->find();
        if ($birthday == null) {
            webApi(400, '参数错误!');
        }
        //添加记录的数据
        ...
        Db::startTrans();
        try {
            Db::table($this->db . '.vip_inbirthday_record')->insert($record);

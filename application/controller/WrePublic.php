 ...
 * Description 公众模板
 */
class WrePublic extends Common
{
    /**
     * 查询公众模板
     */
    public function WrePublicList()
    {
        //分页信息
        ...
        //统计数量
        $count = Db::table($db . '.vip_inpublic_interaction')
            ->alias('v')
            ...
            ->count();
        //所需数据
        $data = Db::table($db . '.vip_inpublic_interaction')
            ->alias('v')
            ->leftJoin($db . '.vip_org vg', 'vg.code = v.org_code')
            ->field('v.*,vg.name vgname')
            ->where($where)
     ...
     * 公众互动记录
     */
    public function WrePublicRecord()
    {
        //生成单号
        $foot = 'GZRHD' . str_replace('.', '', microtime(1));
        //按单号查询模板
        $public = Db::table($this->db . '.vip_inpublic_interaction')->where('code', input('template_code'))->find();
        if ($public == null) {
            webApi(400, '参数错误!');
        }
        //添加记录的数据
        ...
        Db::startTrans();
        try {
            Db::table($this->db . '.vip_inpublic_record')->insert($record);
